...
SELECT models.name AS model_name, brands.name AS brand_name, headquarters FROM models JOIN brands USING (brand_id) WHERE models.name='Mustang';
...
-----
 ...
 WHERE m.name IS NOT NULL ORDER BY b.name;
...
SELECT name, founded FROM brands WHERE brands.brand_id NOT IN (SELECT models.brand_id FROM models);
...
SELECT models.name, models.year FROM models WHERE models.year in (SELECT brands.founded FROM brands);